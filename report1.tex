...
La imagen REFERENCIAL y la imagen PSF a trabajar se presentan como:
...
\end{figure}
 ...
 Y se obtienen las siguientes imágenes resultantes de acuerdo a lo que se pide en el siguiente orden:

\begin{enumerate}

    \item Convolution Noiseless (convolución sin ruido)
    \item Simulation with noise-Gauss (convolución con ruido Gaussiano. Mean=0, Stdev: 0.1 y 0.01
    \item Simulation with noise-Poisson (Convolución con ruido Poisson 0.01, 10 y 0.0001.
    \item Aplicar a las imagenes anteriores Regularized Inverse filter: RIF: =0.01, =1.y Richardson-Lucy (RL) algorithm con 10 y 50 iteraciones. Probar 100 iteraciones para una caso y comparar con 50 iteraciones
